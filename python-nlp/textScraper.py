...
archive_url = "http://textfiles.com/history/"
print(archive_url)
...
def get_text():
    ...
    for item in soup.findAll("td", {"align" : "TOP"}):
        print(item)
        link = item.find('a')
        # print("Items in td element with a element:", link)
        href = archive_url + link['href']
        print(href)
        download_links(href)
    print("All texts downloaded!")
...
def download_links(href):
    ...
    fileDeposit = os.path.join(workingDir, 'textFiles', file_name)
    print(fileDeposit)
